# Usage canonique — odoo_inspect_report
...
- `report_name="account.report_invoice"` : nom technique de l'action de rapport (préfixe module + `report_` + nom court). Reconnaissable depuis l'URL d'impression ou via `odoo_inspect_navigation`.
...
- L'héritage Studio écrit avec un xml_id du style `studio_customization.report_invoice_xxxx_hash` — illisible. Ouvrir avec `odoo_inspect_view` (les rapports sont stockés comme `ir.ui.view`) pour voir l'arch.
