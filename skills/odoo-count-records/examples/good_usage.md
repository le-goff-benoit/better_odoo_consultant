# Usage canonique — odoo_count_records
...
- Pas de `limit` : `odoo_count_records` retourne juste un entier, ça ne coûte rien d'avoir le vrai total.
...
> Si tu veux les voir une par une, je peux les lister avec `odoo_query_records` (préciser combien tu veux récupérer).

**Pièges à éviter** :
- Ne **pas** compter en appelant `odoo_query_records` puis en mesurant la taille du résultat — `odoo_query_records` peut être borné par `max_records` ou limité explicitement.
