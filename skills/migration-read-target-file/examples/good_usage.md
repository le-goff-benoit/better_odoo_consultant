# Usage canonique — migration_read_target_file
...
- `path` commence par `community/` (préfixe standard pour les sources Odoo). Même convention que `source_read_odoo_file`.
- `start_line=1700` : la position est obtenue d'abord via `migration_search_target_source` qui a remonté `def action_post` à la ligne 1735.
...
- Pour comparer ligne à ligne version source vs cible : faire deux appels (`source_read_odoo_file` pour source courante, `migration_read_target_file` pour cible) puis comparer côté assistant.
