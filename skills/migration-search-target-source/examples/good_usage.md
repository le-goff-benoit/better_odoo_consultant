# Usage canonique — migration_search_target_source
...
> ```
>
> Si ton code custom appelle encore `action_invoice_open`, c'est un point bloquant pour la migration 16 → 17. Je peux chercher les call sites dans le repo client avec `repo_search_code` pour évaluer le scope du changement.
...
- Pour le détail des breaking changes par version, croiser avec `odoo_inspect_modules` (modules actuellement utilisés) + notes de version Odoo (`odoo-17.0.md` chargé via le skill `runtime_release_notes_injector`).
