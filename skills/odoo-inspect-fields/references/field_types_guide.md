...
|---|---|---|
| `x_studio_*` | Studio UI | `odoo_inspect_studio` + `state='manual'` dans `ir.model.fields` |
...
- Pour voir l'arch d'une vue (où le champ est exposé) → `odoo_inspect_view`
- Pour le code source d'un compute → `source_read_odoo_file` (besoin du chemin)
- Pour les contraintes/règles applicables → `odoo_inspect_security` (côté ACL/record rules)
