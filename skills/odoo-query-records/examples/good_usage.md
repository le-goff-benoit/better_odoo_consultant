# Usage canonique — odoo_query_records
...
**Pièges à éviter** :
- Ne jamais déduire un total global depuis un résultat `truncated=true` ou avec `limit>0` — utiliser `odoo_count_records` ou `odoo_aggregate_records` pour les chiffres exhaustifs.
