...
Le skill `output_report_writer` a un template `changelog_entry.md` pour formaliser ça.
...
- `source_show_commit scope=odoo|enterprise|target|project` : voir le diff d'un commit donné.
- `source_search_odoo` : retrouver le code source touché par un commit.
- `output_report_writer` (template `changelog_entry.md`) : formater un commit en entrée de changelog.
