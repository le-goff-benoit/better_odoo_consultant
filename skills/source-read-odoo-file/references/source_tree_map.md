...
Le tool `source_search_odoo` retourne les chemins avec un préfixe :
- `community/...` → fichier dans `17.0/`
- `enterprise/...` → fichier dans `17.0-enterprise/`

Quand tu passes ces chemins à `source_read_odoo_file`, conserve le préfixe.
...
- `source_read_odoo_file` : lire un fichier identifié.
- `source_show_commit scope=odoo` : voir un commit Odoo Community.
- `source_show_commit scope=enterprise` : voir un commit Odoo Enterprise.
- Pour le code du projet client (custom), c'est `repo_search_code` et `repo_read_file`.
