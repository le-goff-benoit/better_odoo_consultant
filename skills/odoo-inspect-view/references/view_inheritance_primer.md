...
Souvent un `position="replace"` mal calibré dans un module récent. Diagnostic : `odoo_inspect_view` pour voir la chaîne d'héritage et identifier qui a tué la vue.
...
L'attribut `groups` restreint l'affichage à un groupe. Différent des ACL (`odoo_inspect_security`) qui contrôlent l'accès **données**.

## Outils complémentaires

- `odoo_inspect_view` : voir l'arch assemblée d'une vue et sa chaîne d'héritage.
- `source_search_odoo` : chercher des patterns xpath réels dans le code Odoo standard pour s'inspirer.
- `odoo_inspect_studio` : inventaire des modifications Studio sur le projet.
