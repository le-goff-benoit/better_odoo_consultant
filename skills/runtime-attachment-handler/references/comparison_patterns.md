...
- Comparer une **réponse Odoo live** vs un **PDF** : extraire le PDF puis appeler `odoo_query_records` pour récupérer la version Odoo, comparer côté assistant.
- Comparer **deux versions Odoo** du même devis (historique) : pas de tool dédié — utiliser `odoo_query_records` deux fois (un sur la version courante, un sur la version archivée).
- Comparer un **template QWeb** (XML) vs un **PDF généré** : c'est `odoo_inspect_report` pour le template + extraction PDF de l'autre côté.
