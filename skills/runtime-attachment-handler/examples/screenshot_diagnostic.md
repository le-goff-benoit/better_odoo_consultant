...
> ```json
> { "name": "odoo_query_records", "arguments": { "model": "account.account", "domain": [["code","=","44571"]] } }
> ```
...
→ Enchaîner avec `odoo_inspect_view` sur le modèle visible pour vérifier l'arch après héritage.

### Capture du Studio (config de champ custom)

→ Enchaîner avec `odoo_inspect_studio` pour comparer config visible vs vérité serveur.

### Capture de log Python multi-ligne

→ Identifier le fichier:ligne du dernier `File ".../addons/xxx/yyy.py", line N` → `source_read_odoo_file` ou `repo_read_file` pour montrer le code.

### Capture d'un menu / barre de navigation

→ Si la question est « où je clique pour X » : `odoo_inspect_navigation(model=...)`.
...
- ❌ Ne pas enchaîner avec un tool de vérification quand le diagnostic est testable (toujours croiser avec la vérité serveur via `odoo_query_records` / `inspect_*`).
