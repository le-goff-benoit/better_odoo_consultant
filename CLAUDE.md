...
Dernière mise à jour contextuelle : 2026-05-25, alignée sur l'app `0.92.0` et les derniers commits `main`.
...
- **Eval queries** : **28/28 skills** ont un `eval_queries.json` (depuis 0.92.0, couverture totale). Convention : `expected_skill` informationnel, le test asserte sur le **owner** (dossier → underscore). Une entrée peut porter `"xfail": "raison..."` pour documenter une faiblesse dispatcher connue sans bloquer la CI (0 xfail actif en 0.92.0). Pour les cores avec `allow_implicit_invocation: false`, le positif est typiquement le nom de skill brut + plusieurs négatifs montrant que le mot-clé ne déclenche pas implicitement.
