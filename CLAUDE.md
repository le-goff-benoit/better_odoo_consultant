...
Dernière mise à jour contextuelle : 2026-05-27, alignée sur l'app `0.100.0` et les derniers commits `main`.
...
- `0.100.1` : **trois fixes IA livrés ensemble.** (1) **Suivi systématique des objets liés.** `odoo_query_records/SKILL.md` gagne une section « Suivre les relations » avec un tableau de recettes (parent → enfant + champ relationnel) pour 10 modèles courants (sale.order/sale.order.line, account.move/account.move.line, project.project/project.task, stock.picking/stock.move, mrp.production, hr.employee, res.partner, etc.) ; BA `AGENT.md` FR+EN consigne « charge systématiquement les objets liés ». Cause traitée : les champs `one2many` ramenés par `search_read` ne contiennent que des ids, donc une seconde query sur le modèle enfant est obligatoire — sans instruction explicite, le LLM s'arrête à l'entête. (2) **Fix perte de conversation.** Buffer en mémoire `_msgBuffer` (qui sert à survivre aux unmounts React pendant le streaming) n'était pas synchronisé quand `resumeConv` ou `resetCurrentConversation` modifiaient l'état via `setConversations` directement (et non via `setMessages`). Au mount suivant, l'effet de fusion buffer→état réécrasait le contenu repris par l'ancien. Helper `overwriteCurrentConversation(next)` introduit qui met simultanément à jour `_msgBuffer`, LocalStorage et `setConversations`. (3) **Consigne tool-first sur Support et BA.** Avant : « si tu ne peux pas vérifier, dis-le explicitement » était lu comme une porte de sortie facile → l'IA répondait à l'aveugle au lieu d'appeler `odoo_inspect_*` / `odoo_query_records` / `triage_odoo_error` / `source_read_odoo_file`. Désormais, « d\'abord vérifier, ensuite répondre » est posé comme principe : l'outil prime sur l'hypothèse, « je ne peux pas vérifier » exige le nom de l'outil tenté + raison de l'échec. Pour le BA, garde anti-paradoxe : « tu n\'es pas un inventaire technique » ne libère pas de l\'obligation d\'inspecter — les détails techniques restent input de travail, pas format de sortie. Suite 99 frontend + 324 backend ciblés verts. **Règles dégagées : (a) un LLM qui voit des `one2many` ne sait pas instinctivement qu\'il doit chaîner — il faut une recette explicite ; (b) un buffer en mémoire qui sert de fallback persistance doit être synchronisé sur TOUS les chemins qui modifient l\'état, pas seulement le chemin principal ; (c) une consigne IA qui offre une porte de sortie facile (« sinon dis-le ») est lue comme un repli par défaut — pour pousser à l\'usage des outils, il faut interdire le repli avant tentative explicite.**
- `0.100.0` : **séparateurs HR + liens `odoo://` cliquables + callout « Exemples concrets ».** Trois améliorations pour ancrer les réponses BA dans la donnée client réelle. (1) `---` (et `***`, `___`) rendus comme `<hr>` stylé au lieu de fuiter en texte brut. (2) Nouveau scheme `odoo://<model>/<id>` que le LLM produit en Markdown standard `[libellé](odoo://sale.order/12345)`. `Markdown.tsx` détecte et résout en `https://<db_url>/web#id=12345&model=sale.order&view_type=form` (universel Odoo 15→19, pas de dépendance sur action id). Helper `resolveOdooUri` exporté. `MarkdownActionsProvider` accepte un nouveau prop `odooBaseUrl` ; câblé depuis Assistant.tsx et Migration.tsx. Mode général sans profil → span désactivé avec tooltip explicatif. (3) Nouveau callout « Exemples concrets sur cette base » dans `Markdown.tsx` qui détecte le heading FR/EN et rend la section dans une carte tintée (background `brand10`, bordure gauche `brand`, icône ampoule). Template `business_impact_review.md` exige désormais cette section ; BA `AGENT.md` consigne : `odoo_query_records` limit ≤ 3 pour ramener 1-3 enregistrements réels, citation obligatoire en `odoo://`, jamais d'invention d'id. +10 tests Vitest. Suite frontend 98 verts, backend 822 verts. **Règle dégagée : pour qu'un LLM produise du contenu interactif (liens, callouts), il faut un contrat aussi simple que possible — Markdown standard + scheme custom > syntaxe inventée. Le scheme `odoo://` est intercepté côté rendu et n'a aucun impact sur le pipeline LLM ; c'est uniquement une convention de citation qui survit aux copier-coller en Markdown brut.**
