...
Dernière mise à jour contextuelle : 2026-05-28, alignée sur l'app `0.100.9` et les derniers commits `main`.
...
- `0.100.9` : **volets structurels.** Nouveau composant `VoletBlock.tsx` rendu depuis `:::volet[type] Titre` ... `:::` dans la réponse IA. 6 types : `section` (violet), `tip` (vert), `warning` (ambre), `error` (rouge), `info` (bleu), `note` (gris). Pliable au clic (chevron). Alias FR (`attention`→`warning`, `conseil`→`tip`, `erreur`→`error`, `information`→`info`). Instructions de type-mapping ajoutées au `## Format de sortie` des 4 agents (architect, developer, support, BA) — max 2-3 volets par réponse.
- `0.100.8` : **tool-first enforcement — renforcement systématique sur les 4 agents.** Cinq changements coordonnés : (1) **Section `## Catégories tool-obligatoires`** ajoutée dans les 4 agents (architect, developer, support, BA) — tableau explicit par type de question + outil forcé + cas où la mémoire d'entraînement est acceptable, toujours labellisée. (2) **Règle `Résultat outil = source de vérité`** ajoutée dans les consignes transverses des 4 agents : verbatim obligatoire, interdiction d'ajuster silencieusement les données retournées par un outil. (3) **Architect** : gate « D'abord vérifier, ensuite recommander » ajouté (aligné sur Support/BA), `source_read_odoo_file` et `odoo_inspect_modules` ajoutés dans `preferred_skills`. (4) **Developer** : gate vérification étendu aux questions conceptuelles (pas seulement aux patches) — `source_search_odoo` obligatoire avant d'affirmer un comportement standard. (5) **Disambiguation tables `Choisir entre X et les skills voisins`** ajoutées dans 5 SKILL.md clefs (`odoo_inspect_view`, `odoo_inspect_studio`, `odoo_query_records`, `source_search_odoo`, `repo_search_code`) pour réduire l'évitement d'outil par ambiguïté. (6) **10 cas d'éval `tool_required`** ajoutés dans `agent_response_eval.jsonl` (2 architect, 3 developer, 2 support, 3 BA) — vérifient que le bon skill est sélectionné pour les questions qui « semblent simples » mais exigent un outil. Suite 224 cas, agent_accuracy 87.1 %, tool_accuracy 90.6 %, 4 golden_failures. **Règles dégagées : (a) une liste explicite de types de questions tool-obligatoires est plus efficace qu'une exhortation générale « utilise les outils » — le LLM a besoin d'une décision, pas d'un conseil ; (b) les portes de sortie implicites (« si tu peux répondre de mémoire, fais-le ») sont lues comme des permissions par défaut — les remplacer par des catégories explicites autorisées ; (c) l'ambiguïté entre skills similaires est la première cause d'évitement — des tables de désambiguaïsaïon dans les SKILL.md réduisent la friction de sélection.**
- `0.100.7` : **sept améliorations UX et IA livrées ensemble.** (1) **Bouton « Citer »** dans le flottant de sélection de texte (`SelectionAskMore`) : à côté de « Plus de détails », un bouton « Citer » insère un bloc blockquote `> texte sélectionné` dans le prompt input que l'utilisateur peut compléter avant d'envoyer — pas de soumission automatique. (2) **Cap avis complémentaire réduit** : `buildSecondOpinionPrompt` passe de 4 000 à 800 chars pour ne pas alourdir l'historique visible. (3) **Fix boutons d'action « Étapes suivantes »** : `ACTION_HEADING_RE` corrigé pour les non-ASCII (`É`) via lookbehind ASCII-safe au lieu de `\b`, et `•` ajouté à `UNORDERED_LIST_RE` pour les listes bullet en caractère Unicode. (4) **Sanitiseur Mermaid** dans `MermaidBlock` : remplace `\n` littéral dans les labels par `<br/>`, supprime le garbage après `]`, élimine CJK/emoji dans les labels. (5) **BA agent — classification Standard/Custom/Mix** : badge obligatoire en tête de réponse (🟢/🟠/🔵), définition explicite « module custom = dépôt client, pas forcément application », section « Étapes suivantes » imposée en format de sortie. (6) **generate-diagram amélioré** : instructions étape-par-étape pour les diagrammes flux + impact module custom (flux standard → inspection repo → diagramme enrichi avec classDef colorés). (7) **module_taxonomy.md** : note explicite « custom ≠ application », modules issus du dépôt client identifiables via `repo_list_modules`. **Règles dégagées : (a) `\b` en regex JavaScript ne matche pas avant les caractères non-ASCII (`É`/`é`) — utiliser un lookbehind ASCII-safe `(?<![a-zA-Z])` ; (b) un LLM génère souvent `\n` comme séquence littérale dans les labels Mermaid, et `•` au lieu de `-` pour les listes — les deux nécessitent un sanitiseur côté rendu et des instructions explicites côté skill ; (c) un badge de classification (Standard/Custom/Mix) en tête de réponse IA résout immédiatement l'ambiguïté de périmètre sans alourdir la réponse.**
- `0.100.6` : **Détecteur de complexité — modules vendorés.** Un client qui vendre des submodules OCA/community dans son repo (`git submodule`) était silencieusement ignoré → mode `standard` → bloc Contexte projet vide. Désormais `_repo_summary` retourne aussi `vendored_manifest_count` + `vendored_modules`. À partir de 3 manifests vendorés, `dev_detected` passe à `True`. +4 tests. Suite 829 verts.
- `0.100.5` : corrections diverses.
- `0.100.1` : **trois fixes IA livrés ensemble.** **séparateurs HR + liens `odoo://` cliquables + callout « Exemples concrets ».** Trois améliorations pour ancrer les réponses BA dans la donnée client réelle. (1) `---` (et `***`, `___`) rendus comme `<hr>` stylé au lieu de fuiter en texte brut. (2) Nouveau scheme `odoo://<model>/<id>` que le LLM produit en Markdown standard `[libellé](odoo://sale.order/12345)`. `Markdown.tsx` détecte et résout en `https://<db_url>/web#id=12345&model=sale.order&view_type=form` (universel Odoo 15→19, pas de dépendance sur action id). Helper `resolveOdooUri` exporté. `MarkdownActionsProvider` accepte un nouveau prop `odooBaseUrl` ; câblé depuis Assistant.tsx et Migration.tsx. Mode général sans profil → span désactivé avec tooltip explicatif. (3) Nouveau callout « Exemples concrets sur cette base » dans `Markdown.tsx` qui détecte le heading FR/EN et rend la section dans une carte tintée (background `brand10`, bordure gauche `brand`, icône ampoule). Template `business_impact_review.md` exige désormais cette section ; BA `AGENT.md` consigne : `odoo_query_records` limit ≤ 3 pour ramener 1-3 enregistrements réels, citation obligatoire en `odoo://`, jamais d'invention d'id. +10 tests Vitest. Suite frontend 98 verts, backend 822 verts. **Règle dégagée : pour qu'un LLM produise du contenu interactif (liens, callouts), il faut un contrat aussi simple que possible — Markdown standard + scheme custom > syntaxe inventée. Le scheme `odoo://` est intercepté côté rendu et n'a aucun impact sur le pipeline LLM ; c'est uniquement une convention de citation qui survit aux copier-coller en Markdown brut.**
- `0.99.4` : **validation backend défensive des IDs de modèle.** Bien que 0.99.1 ait retiré `gpt-5.5` du catalogue front, des bundles utilisateurs en cache continuaient à POSTer cet ID au chat endpoint → `400 unsupported_api_for_model` côté OpenAI. Nouvelle constante `KNOWN_MODELS` dans `backend/services/ai_service.py` qui mirror le catalogue front pour openai / claude / gemini. Helper `_validate_or_fallback_model(provider, requested)` appelé en tête de `stream_chat` : ID inconnu → fallback sur `DEFAULT_MODELS[provider]` + event SSE `model_fallback` avec raison explicite. Github/Copilot passent sans validation (catalogue live côté API). +7 tests `tests/test_model_validation.py`. Suite 822 verts. **Règle dégagée : un catalogue de modèles côté frontend ne suffit pas — il faut une validation backend défensive en parallèle, parce que le bundle JS peut rester en cache utilisateur pendant des jours après une release. La validation backend doit être un mirror du catalogue front pour les providers statique-only ; pour les providers live (github/copilot), elle ne peut pas l'être, donc on laisse passer.**
- `0.99.3` : **bugfix rendu sous-bullets dans `frontend/src/components/Markdown.tsx`.** La regex de détection des items de liste (`^[-*]\s+(.+)`) était ancrée en tout début de ligne sans tolérer l'indentation. Quand le LLM produisait `  - sub item` (2 espaces + tiret) pour nidifier des sous-bullets sous un parent, la ligne tombait dans le rendu `<p>` au lieu du rendu liste, et le tiret apparaissait comme texte brut décollé du parent. Fix : nouvelles regex `UNORDERED_LIST_RE = /^(\s*)[-*]\s+(.+)/` + `ORDERED_LIST_RE = /^(\s*)(\d+)\.\s+(.+)/`, helper `indentDepth(indent)` (2 espaces ou 1 tabulation = 1 niveau, cap à 3), rendu avec `paddingLeft: depth * 18px` et marqueur creux `◦` aux niveaux ≥1 pour visuellement distinguer les sous-bullets des principaux. +2 tests régressifs (`Markdown.test.tsx`). **Règle dégagée : un renderer Markdown custom doit toujours tolérer l'indentation des items de liste — le LLM utilise l'indentation comme convention sémantique de nidification, et l'ignorer fait fuiter le caractère `-` en texte brut dans la sortie.**
- `0.99.2` : **fix BA framing + scaffolding template qui fuit.** Un BA qui demandait « y a-t-il des modifs Studio sur cette instance » récupérait le template `technical_review` (file:ligne, slots Python, blockquotes scaffolding `> Ce qui empêche la mise en prod. Vide si rien.` recopiées telles quelles dans la sortie). Deux problèmes corrigés ensemble. (1) `technical_review.md` nettoyé : blockquotes d'instruction supprimées, bloc directives en `<!-- ... -->` HTML comment, slot « Correction proposée » conditionnel. (2) Nouveau template `business_impact_review.md` orienté flux métier (Ventes / Achats / SAV / Stock / Facturation), tableau structurant Flux × Modules × Type de personnalisation × Impact utilisateur, pas de Python ni de file:line. (3) Sélection agent-aware via nouveaux champs `preferred_agents` / `forbidden_agents` sur `SkillTemplate` (cf. `backend/skills/registry.py`). `OutputRenderer.select_template` accepte un param `agent` ; score boosté `+50` quand le template est preferred ; templates avec `forbidden_agents` skippés. `business_impact_review` preferred BA / forbidden dev ; `technical_review` preferred dev+architect / forbidden BA. Trigger « audit » sur les deux templates → l'agent actif tranche. Câblé depuis `context_service.py` qui propage `perspective` à toutes les call sites de `select_output_template_details`. (4) BA AGENT.md (FR+EN) explicite : « tu n'es pas un inventaire technique » — traduis les faits techniques en impact métier, redirige vers dev/architect si tech inventaire vraiment voulu. (5) Recalibrage tableaux dans `ai_service.py` + AGENT.md : 1-2 tableaux structurants par réponse quand on croise deux dimensions, plus l'interdiction trop dure de 0.98.0. **Règle dégagée : un template de sortie n'est jamais agent-neutre — un BA et un dev attendent des formats fondamentalement différents même sur la MÊME question (« audit Studio »). La sélection de template doit accepter l'agent comme paramètre de premier plan, pas comme heuristique secondaire. Les blockquotes `> ...` dans un template sont systématiquement reproduites par le LLM ; pour des directives auteur invisibles, utiliser `<!-- ... -->` (HTML comment) — visible dans le source mais non reproduit en sortie.**
- `0.99.1` : **bugfix catalogue modèles hallucinés** — le catalogue statique OpenAI dans `frontend/src/constants/providers.ts` contenait `gpt-5.5`, `gpt-5.5-pro`, `gpt-5.4*`, `gpt-5.3-codex` qui ne sont pas dans le catalogue réel `/v1/models`. `gpt-5.5` était `recommended: true` → les utilisateurs qui suivaient le défaut tombaient sur `400 unsupported_api_for_model`. Catalogue OpenAI nettoyé (IDs vérifiés uniquement : gpt-5, gpt-5-mini, gpt-4o nouveau recommandé, gpt-4o-mini, gpt-4.1, gpt-4.1-mini, gpt-4.1-nano, o3, o3-mini, o4-mini). Gemini idem (sunset 1.5-* début 2026, nouveau recommandé gemini-2.5-pro). Copilot statique resserré (fallback offline). **Régression connexe** : `buildConfiguredProviders` synthétisait les IDs inconnus pour TOUS les providers, donc un ID périmé sauvé dans `model-config.json` survivait au nettoyage du catalogue et réapparaissait dans le sélecteur d'Assistant/Migration/Creator. Désormais nouveau export `LIVE_FETCH_PROVIDERS = new Set(['github', 'copilot'])` — la synthèse est restreinte à ces deux providers (qui ont du live fetch depuis 0.98.1). Pour openai/claude/gemini, la liste statique est la source de vérité et les IDs inconnus sont silencieusement écartés. Bonus : `toggleModel` dans Settings purge l'état stocké contre `allIds` dès qu'un utilisateur interagit avec le sélecteur, nettoyant `model-config.json` à la volée. +2 tests régressifs. Suite frontend 86 verts, backend 809 verts. **Règle dégagée : un catalogue statique de modèles est une dette qui pourrit silencieusement — les modèles évoluent côté provider, le statique drift. La règle est : (1) ne lister que des IDs vérifiables ; (2) pour les providers qui exposent `/v1/models`, fetch live et garder le statique comme fallback minimal ; (3) tout ID inconnu côté provider statique-only doit être ignoré, pas synthétisé.**
- `0.99.0` : **refonte UX Settings → API & Modèles.** Avant : deux blocs disjoints (cartes Configurés en haut + grille Modèles disponibles en bas), bouton Save global en bas, chips compactes sans contexte (label seul). Après : carte unifiée par provider qui contient clé API + sélecteur de modèles collapsible dans la même carte. Sélecteur enrichi : badge `live`/`liste statique`, bouton refresh, actions bulk (Tout cocher / ★ Recommandés seulement / Tout décocher), lignes riches (checkbox + ★ doré recommandé + label + description courte + tags), regroupement par famille pour GitHub Models et Copilot (OpenAI / Anthropic / Google / Mistral / Meta Llama / Microsoft Phi / DeepSeek / xAI Grok / Cohere / AI21 / Autres). **Autosave optimiste** debounce 500ms : chaque toggle programme un POST, indicateur intégré `⏳ enregistrement…` → `✓ enregistré` qui s'estompe à 1,8s. Nouveau helper `inferFamily(id)` + champ optionnel `family?: ModelFamily` sur `ModelDef` dans `frontend/src/constants/providers.ts` — utile pour grouper les IDs live-only que GitHub/Copilot retournent et qui ne sont pas dans la liste statique. Suite frontend 84 verts (24 nouveaux cas `inferFamily`), backend 809 verts. **Règle dégagée : un sélecteur de modèles à liste plate avec save global au pied de page passe à l'échelle jusqu'à ~10 modèles ; au-delà (GitHub Models en a 30+), il faut absolument grouper + autosave + actions bulk. La règle bascule à partir du moment où l'utilisateur scrolle pour atteindre le bouton Save.**
- `0.98.2` : **bugfix critique — régression « Aucun fournisseur IA configuré » introduite par 0.98.1.** Cause racine : 0.98.1 ajoutait les modèles **live** GitHub/Copilot dans Settings (ex. `gpt-4o-2024-11-20`, `Mistral-Large-2411`) sans présence dans la liste statique `PROVIDERS` (`frontend/src/constants/providers.ts`). Quand l'utilisateur cochait ces IDs live-only, ils étaient persistés dans `model-config` ; puis sur Assistant/Migration/Creator, le filtre inline `p.models.filter(m => enabled.includes(m.id))` intersectait avec la liste statique, retournait `[]`, et le provider entier était drop par `.filter(p => p.models.length > 0)` → sélecteur vide, message « Aucun fournisseur IA configuré — ajouter une clé API → ». Fix : nouveau helper `buildConfiguredProviders(allProviders, modelConfig)` dans `frontend/src/constants/providers.ts` qui synthétise les IDs live-only en entrées `{id, label: id, desc: ''}` au lieu de les jeter. Les 3 pages utilisent désormais ce helper au lieu de leur filtre inline. 7 tests Vitest dans `providers.test.ts` verrouillent le comportement (suite frontend 60 verts, backend 809 verts). **Règle dégagée : quand on étend ce que l'utilisateur peut cocher dans une UI de config, toujours vérifier les lecteurs en aval — un filtre `enabled.includes(m.id)` paraît robuste mais il intersecte silencieusement avec un catalogue statique, et à la moindre divergence statique↔live, la sélection entière peut disparaître sans message d'erreur.**
- `0.98.1` : **modèles GitHub Models et Copilot chargés en live depuis les APIs.** Avant : liste codée en dur dans `frontend/src/constants/providers.ts`, identique pour tous les abonnements. Désormais : deux endpoints backend `GET /api/ai/github/models` (appelle `models.inference.ai.azure.com/v1/models` avec le token GitHub stocké) et `GET /api/ai/copilot/models` (échange OAuth → token Copilot puis appelle `api.githubcopilot.com/models`) — ajoutés dans `backend/api/routes/ai.py`. `ModelConfigEditor` dans `frontend/src/pages/Settings.tsx` refactorisé : deux `useQuery` avec `staleTime` 5 min et `retry: false` ; fusion des ids live avec `STATIC_LABEL` (métadonnées statiques) — les modèles connus gardent leur label, les inconnus affichent l'id brut + marqueur `·new` ; badge **live** quand les données viennent de l'API, mention **liste statique** en cas d'erreur ; bouton refresh sans rechargement. Fallback gracieux vers la liste statique si l'API est inaccessible (réseau, token expiré, plan insuffisant).
- `0.98.0` : **qualité des réponses IA — trois axes adressés ensemble.** (1) **Bloc « Contexte projet » prioritaire unifié** dans `runtime-complexity-analyzer` (non tronqué, cap 6 000 chars) : client + ville, localisation fiscale, profil technique calculé avec confiance, Studio/dev custom détectés, modules custom installés, apps Odoo installées, stratégie de réponse selon le mode. Quand la complexité n'a pas été calculée, affiche explicitement « non calculée — lancer le diagnostic » plutôt que de laisser supposer une base vanilla. Nouvelle fonction `build_project_context_block` dans `technical_complexity_service.py:398`, câblée dans `ai.py:865` et `creator.py:178`. Renommage du free-text consultant de `## Contexte projet` à `## Notes projet (consultant)` pour ne plus collisionner. (2) **Consignes preuves + step further dans les 4 AGENT.md (FR + EN)** : preuve obligatoire pour toute affirmation technique (fichier:ligne, nom de champ/modèle, ID de vue, SHA, ligne de traceback — ou dire qu'on ne peut pas vérifier). Conscience contexte projet exige de lire le bloc prioritaire et de mentionner la couche custom/Studio. Ton support = précis/actionnable/rapide avec alerte explicite quand la correction est superficielle ; ton BA = pédagogique (jamais oui/non sans concept + mode opératoire + limites + cas d'usage) ; architect et developer gardent leur ton actuel. (3) **Règle tableaux durcie** dans `ai_service.py:2024-2035` et build_system_migration/general : tableau uniquement pour ≥3 éléments comparables sur ≥2 dimensions, pas de chaînage. Le mode migration garde le tableau comparatif (le bon cas d'usage). Chaque AGENT.md ajoute une ligne layout dans Format de sortie. **Harnais éval** : 14 nouveaux cas qualité dans `agent_response_eval.jsonl` avec tokens `cite:*` / `step:*` / `context:*` dans `criteria` ; scoring déterministe `criteria_hits` (9 regex patterns) + métrique `avg_table_count` ajoutés dans `run_agent_response_eval.py`. Les 14 cas sont `golden=false` pour servir de markers de progression sans polluer la métrique golden_failures (toujours 4). Suite 809 passed. **Règle dégagée : préférer un bloc structuré unique au sommet du system prompt (« ## Contexte projet ») plutôt qu'éparpiller métier/localisation/complexité dans des sections routées budgetées — le contexte projet doit être visible avant même que l'IA commence à raisonner, sinon il n'a pas d'effet sur la réponse.**
- `0.97.4` : **bugfix édition profil — la clé API n'est plus écrasée quand l'utilisateur sauvegarde sans la modifier.** Cause racine dans `backend/api/routes/profiles.py::update_profile` : `model_dump(exclude_none=True)` exclut `None` mais pas la chaîne vide, donc `store_profile_secrets(name, "")` était appelé et wipait le secret. L'UI annonçait « clé API inchangée » mais le prochain appel IA échouait silencieusement. Fix une ligne (garde `if v:` autour de `store_profile_secrets`) alignée sur le comportement déjà correct de `update_environment`. Ceinture+bretelles côté frontend : la mutation `update` de `Profiles.tsx` retire `api_key` du payload quand le champ est vide. +2 tests régressifs (PATCH `api_key=""` ET PATCH omettant `api_key` → secret préservé ; PATCH avec nouvelle valeur → propagée). **Règle dégagée : tout endpoint qui écrit dans le secret store doit garder explicitement contre la chaîne vide en plus de `None` — `exclude_none` ne suffit pas.**
- `0.97.3` : **résolution des 4 faiblesses dispatcher identifiées en 0.97.2**, toutes documentées en `xfail` avec le wording exact du fix nécessaire. Cause racine du cluster d'erreurs : le keyword anglais `list` de `odoo_inspect_view` (= "list view" en mode UI) tombait en substring sur le verbe français "liste". Ajout à `_BOUNDARY_TOKENS` pour passer en word-boundary matching — un seul changement de 5 caractères qui élimine une catégorie entière de faux positifs. Trois patterns/rules ajoutés au dispatcher : (1) `manifest-read-pattern` au scoring (score 100) détecte `verbe-lecture + __manifest__.py` et route vers `source_read_odoo_file` ou `repo_read_file` selon un signal projet/custom, avec prune corrélée de `repo_list_modules`/`inspect_module_graph`/`odoo_inspect_modules` ; (2) `pruned:sha-focus` élimine les skills source frères quand `source_show_commit` est sélectionné via SHA `\b[0-9a-f]{7,40}\b`, **avec préservation de `repo_*` quand le prompt mentionne un override/projet client** (sinon les goldens commit-analysis cassent — leçon : tester l'agent eval en plus du dispatcher eval à chaque modif de `_prune_skill_routes`) ; (3) `query-focus` étendu pour pruner aussi `odoo_inspect_view/navigation/security` quand des marqueurs list-detail accompagnent une intention query. `financial_terms` enrichi de `rapport tva`/`vat report`/`rapport balance`/`rapport grand livre`/`trial balance` pour que la prune `financial-report-focus` se déclenche sur les vrais phrasings (avant elle ne matchait que `déclaration tva`/`tax report`). Résultats : Skill dispatcher 311 cas à 100 %/100 %/100 %/100 %, **0 known weakness restante**, Agent response eval inchangé à 90.5 %/98.5 %/4 goldens, suite 807 passed. **Leçon dégagée : les xfails avec wording exact du fix sont un meilleur livrable qu'une rustine — au moment où on revient pour patcher, on ne devine plus la classe de fix nécessaire, elle est déjà écrite à côté du cas qui la révèle.**
- `0.97.2` : **désambiguïsation des paires de skills sémantiquement proches**. Méthode : exploiter le `_SemanticIndex` déjà construit pour les skills (TF-IDF over `description + description_en + keywords + name`) afin de calculer les paires cosine-proches (top-20 ≥0.20, dominées par les triplets read/search/modules entre source/repo/migration). Pour les 11 paires les plus à risque, 27 cas de désambiguïsation écrits manuellement dans `scripts/quality_eval/disambiguation_cases.jsonl` (3 cas par paire : A-gagne / B-gagne / variant), avec un champ nouveau `forbidden_skills` qui force une assertion stricte « X DOIT être sélectionné ET tels autres NE DOIVENT PAS être sélectionnés ». Le harnais `run_skill_dispatcher_eval.py` charge ce fichier comme troisième source et le rapport sépare désormais `## Real failures` (régressions à fixer) de `## Known weaknesses (xfail, excluded from scoring)` (faiblesses dispatcher documentées avec la classe de fix nécessaire). Un fix appliqué (keywords supplémentaires sur `odoo_inspect_modules` : `installé en base`, `actuellement installé`, etc.) ; 4 cas marqués xfail avec wording du fix à appliquer plus tard : bundle `intent:record_analysis` trop large (p04b), pattern `verbe+__manifest__.py` non câblé (p06b), over-selection sur SHA (p10a), `odoo_inspect_report` co-sélectionné avec `inspect_financial_reports` sur TVA (p11a). Résultat : 307 cas, accuracy 100 % hors xfail, train/dev/test tous à 100 %, suite 807 passed. **Règle dégagée : utiliser le semantic index existant comme oracle de proximité pour générer les cas de désambiguïsation (les paires sont issues du dispatcher lui-même, pas d'intuition humaine arbitraire) ; le statut `xfail` avec wording exact du fix est un meilleur livrable qu'une rustine bricolée qui passerait par chance.**
- `0.97.1` : **boucle de feedback portée du routing agents au skill dispatcher**. Nouveau harnais unifié `scripts/quality_eval/run_skill_dispatcher_eval.py` qui ingère les 28 prompts curated de `dataset.json` + les 256 queries des `skills/*/eval_queries.json` (284 cas), avec split stable train/dev/test 70/15/15 (`sha1(id) % 100`), exécution multi-modes (un cas `modes: [migration]` route en mode migration, comme `test_trigger_routing`), matrice de confusion skill-to-skill, paraphrase accuracy, exclusion des cas `xfail` du scoring. 24 paraphrases ajoutées sur 12 skills pivots ; mesure initiale 91.7 %, deux trous détectés et bouchés : `odoo_aggregate_records` enrichi de `ca mensuel`/`mensuel`/`annuel`/`trimestriel`/`monthly`/`yearly`/`réalisé`/`tendance`, `triage_odoo_error` enrichi de `exception python`/`exception remonte`/`cause racine`/`does not exist`/`peux-tu la classer`. Résultat 100 % accuracy + 100 % paraphrase sur le corpus complet, suite 807 passed. Pin sur une vérité utile : **ajouter des keywords à une SKILL.md modifie aussi le vote sémantique TF-IDF (`semantic_router`), pas seulement le scoring lexical** — un cas qui dépendait du semantic fallback (cos ≈0.13) peut décrocher si on dilue le corpus. Toujours mesurer baseline + post-edit sur le harnais avant de commit.
