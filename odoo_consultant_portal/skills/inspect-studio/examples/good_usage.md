# Usage canonique — inspect_studio
...
**Pièges à éviter** :
- Studio stocke ses changements dans le module `studio_customization` (`ir.model.data`). Si le projet a aussi des modules custom code, les deux **coexistent** — `inspect_studio` ne voit que Studio.
...
- Certaines vues Studio sont stockées en mode `mode='extension'` avec xpath — vérifiables via `inspect_odoo_view` pour comprendre ce qu'elles modifient.
- Les modèles `x_studio_*` ne supportent **pas** toutes les vues (cf. `inspect_studio/references/studio_limits.md`).
