...
- `inspect_odoo_report` : voir la structure d'un rapport, ses héritages, son paperformat.
- `inspect_odoo_view` : les templates QWeb sont stockés en `ir.ui.view` — accessibles via cet outil.
- `search_odoo_source` + `file_types=["*.xml"]` : chercher des patterns QWeb dans le code Odoo standard pour s'inspirer.
