# Usage canonique — read_odoo_file
...
- `path` commence par `community/` : le préfixe est imposé par `search_odoo_source` quand on est dans le code standard. Sans préfixe, le tool essaie `community` puis `enterprise`.
- `start_line=230` : on a localisé la méthode à la ligne 236 via `search_odoo_source`, on prend une marge avant pour voir le décorateur `@api.depends`.
