...
description: "Gère les fichiers PDF et images uploadés par le consultant : routage provider-aware (Claude/Gemini natif, OpenAI/GitHub/Copilot via fallback texte+image), extraction, conversion et comparaison."
description_en: "Handles PDF + image attachments uploaded by the consultant: provider-aware routing (Claude/Gemini native, OpenAI/GitHub/Copilot via text+image fallback), extraction, conversion, and comparison."
...
permissions:
  ...
  scripts: true
  odoo: none
code_path: odoo_consultant_portal/skills/attachment-handler/scripts/handler.py
...
- **Facture fournisseur → vendor bill draft** : `extract_pdf_text` → parse montants/TVA/fournisseur → `query_odoo` pour matcher le partenaire → proposer un `account.move` à valider.
...
- **Capture d'écran d'erreur Odoo** : vision native du LLM → identifier le composant (vue cassée, traceback, log) → enchaîner avec `inspect_odoo_view` ou `inspect_security` selon le diagnostic.
- **Plan comptable PDF** → croiser avec `account.account` Odoo via `query_odoo`, signaler comptes manquants.
...
- **Logo client** → suggérer son intégration dans le QWeb `external_layout` via le skill `inspect_odoo_report`.
- **Capture du Studio** → comparer la config visible avec `inspect_studio` pour repérer les écarts.
