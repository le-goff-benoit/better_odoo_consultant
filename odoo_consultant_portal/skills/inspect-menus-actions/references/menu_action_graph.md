...
Diagnostic « pourquoi l'utilisateur voit/ne voit pas X » : croiser `inspect_menus_actions` + `inspect_security`.
...
- Une action existe **sans menu** si elle est appelée seulement depuis un bouton ou un programme. Le champ `menus` retourné par `inspect_menus_actions` est souvent vide pour ce cas — c'est normal.
...
- Studio crée des actions avec des xml_id du type `studio_customization.xxx_yyy` — pas toujours lisibles. Préférer `inspect_studio` pour les inventorier.
