...
def cli():
    """Better Odoo Assistant — local AI assistant for Odoo consultants."""
...
def init():
    ...
    console.print("\nRun [bold]better-odoo-assistant serve[/bold] to start the web UI.")
...
def serve(host, port, reload):
    ...
    console.print(f"[bold green]Starting Better Odoo Assistant at http://{_host}:{_port}[/bold green]")
    uvicorn.run(
        "backend.api.app:app",
        host=_host,
        port=_port,
