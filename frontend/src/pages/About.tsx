...
const CHANGELOG = [
  ...
  },
  {
    ...
    items: [
      ...
      'AGENT.md des 4 agents (FR + EN) — consigne preuves obligatoire ajoutée dans Comportement : toute affirmation technique doit pointer fichier:ligne, nom de champ/modèle, ID de vue, SHA de commit, ligne de traceback (ou dire explicitement qu\'on ne peut pas vérifier). Conscience contexte projet rappelle de lire le bloc prioritaire avant de répondre et de mentionner explicitement la couche custom/Studio quand elle altère le standard. Pour support : ton précis/actionnable/rapide avec alerte explicite quand la correction immédiate ne traite que le symptôme (problème plus profond côté custom/Studio). Pour business_analyst : posture pédagogique — jamais oui/non sans concept + mode opératoire + limites + cas d\'usage, parce que le consultant monte en compétences en utilisant l\'outil. Architect et developer : preuves + contexte ajoutés sans changer leur ton déjà bon',
      'Layout — règle tableaux durcie dans `ai_service.py` (build_system + build_system_general + build_system_migration) : préférer texte fluide avec citations en ligne et bullet lists, tableau **uniquement** pour ≥3 éléments comparables sur ≥2 dimensions, pas de chaînage de tableaux. Le mode migration garde le tableau comparatif Source/Cible (c\'est le bon cas d\'usage) mais limité à un seul tableau structurant par réponse. Chaque AGENT.md ajoute une ligne layout dans Format de sortie — pour business_analyst en particulier, le texte pédagogique est préféré au tableau dépouillé. Renommage du bloc free-text consultant de `## Contexte projet` à `## Notes projet (consultant)` pour ne plus collisionner avec le nouveau bloc prioritaire',
      'Harnais agent response eval — 14 nouveaux cas qualité ajoutés dans `agent_response_eval.jsonl` avec 3 catégories de tokens dans `criteria` : `cite:file_line`/`cite:field_name`/`cite:view_id`/`cite:sha`/`cite:menu` pour la discipline de preuves, `step:howto`/`step:limits` pour le step-further support/BA, `context:mentions_custom_layer`/`context:flags_not_computed` pour la contextualisation projet. Nouveau scoring déterministe `criteria_hits` dans `run_agent_response_eval.py` : 9 patterns regex matchent les preuves dans une trace de réponse externe (loaded via `--responses`), avec status `graded`/`pending`/`miss`/`hit` par token. Nouvelle métrique `table_count` qui mesure le nombre de tableaux Markdown par réponse (cible support/BA < 1, architect/dev < 1.5). Les 14 cas sont marqués `golden=false` pour servir de markers de progression sans polluer la métrique golden_failures (toujours à 4). Suite 809 passed',
      'Bugfix bonus — `update_profile` utilisait `model_dump(exclude_none=True)` qui n\'exclut pas la chaîne vide : la clé API du profil était wipée en silence quand l\'utilisateur sauvegardait sans toucher au champ (placeholder « inchangée »). Fix une ligne (garde `if v:` autour de `store_profile_secrets`) + ceinture+bretelles côté `Profiles.tsx` qui retire `api_key` du payload quand vide. +2 tests régressifs (voir 0.97.4)',
    ],
  },
  {
    version: '0.97.4',
