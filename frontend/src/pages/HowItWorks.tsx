...
const steps: DiagramStep[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Il charge le guide de profil consultant adapté, les notes de version Odoo si la version est active, Studio si besoin, le modèle de compte-rendu si la demande le suggère.",
    ...
    technicalRefs: ['load_context_for_prompt()', '_markdown_sections()', '_fit_context_budget()', 'routedContextFiles()', 'CONTEXT_BUDGET=36000'],
    inputs: ['Dernier prompt', 'Perspective', 'Version', 'Mode migration'],
    outputs: ['Contexte compact ≤ 36 k', 'Budget maîtrisé', 'Signal profil + notes + Markdown'],
  },
  {
    ...
    details: [
      "Le système assemble : identité consultant, perspective, instance Odoo, complexité technique, sources disponibles, contexte Markdown routé, contexte projet et langue de réponse souhaitée.",
      "Les outils exposés varient selon le mode : données Odoo live (search_read, get_fields…), code source Odoo, repo client, Studio (inspect_studio_customizations), comptage de lignes.",
      "L'IA est instruite d'utiliser les outils pour vérifier les faits plutôt que de répondre uniquement depuis sa mémoire d'entraînement — en particulier pour les champs, modèles et comportements version-spécifiques.",
    ],
    technicalRefs: ['build_system()', 'build_system_general()', 'build_system_migration()', 'stream_chat()', '_perspective_block()', '_complexity_block()'],
    inputs: ['Contexte routé', 'Projet + complexité', 'Sources', 'Provider', 'Modèle', 'Langue'],
    outputs: ['Prompt système assemblé', 'Tools disponibles', 'Appel provider'],
  },
  {
    title: '13. Réponse IA',
    ...
    details: [
      "La réponse est streamée dans l'interface avec les appels d'outils et résultats visibles quand ils existent, garantissant la traçabilité des vérifications effectuées.",
      ...
      "Les conversations peuvent être conservées dans l'historique et réutilisées pour générer un compte-rendu de réunion formaté en Markdown.",
    ],
    technicalRefs: ['SSE / chat stream', 'tool_call / tool_result', 'Historique conversation', 'meeting-minute.md'],
...
const qualityTips = [
  ...
  "Utiliser le mode Auto de la perspective : il détecte Support, BA, Architecte ou Développeur à partir de vos mots-clés et des signaux forts (code, traceback, _inherit).",
...
const stepsEn: DiagramStep[] = [
  ...
  },
  {
    ...
    details: [
      ...
      'It loads the appropriate consultant profile guide, Odoo version notes when the version is active, Studio if needed, and the meeting-minute template when the request calls for it.',
    ...
    outputs: ['Compact context ≤ 36 k', 'Controlled budget', 'Profile + notes + Markdown signal'],
  },
  {
    ...
    badges: ['Provider', 'Tool', 'Odoo', 'Repo'],
    details: [
      'The system assembles: consultant identity, perspective, Odoo instance, technical complexity, available sources, routed Markdown context, project context and desired response language.',
      'Exposed tools vary by mode: live Odoo data (search_read, get_fields…), Odoo source code, client repo, Studio (inspect_studio_customizations) and line counting.',
      'The AI is instructed to use tools to verify facts rather than answer from training memory alone — especially for version-specific fields, models and behaviors.',
    ],
    technicalRefs: ['build_system()', 'build_system_general()', 'build_system_migration()', 'stream_chat()', '_perspective_block()', '_complexity_block()'],
    inputs: ['Routed context', 'Project + complexity', 'Sources', 'Provider', 'Model', 'Language'],
    outputs: ['Assembled system prompt', 'Available tools', 'Provider call'],
  },
  {
    title: '13. AI answer',
    ...
    details: [
      ...
      'Conversations can be kept in history and reused to generate a formatted Markdown meeting-minute report.',
    ],
    technicalRefs: ['SSE / chat stream', 'tool_call / tool_result', 'Conversation history', 'meeting-minute.md'],
...
const qualityTipsEn = [
  ...
  'Use Auto mode for the perspective: it detects Support, BA, Architect or Developer from your keywords and strong signals (code, traceback, _inherit).',
