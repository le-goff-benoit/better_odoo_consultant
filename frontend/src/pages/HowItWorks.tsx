...
const steps: DiagramStep[] = [
  ...
  },
  {
    ...
    details: [
      ...
      "Chaque agent possède un AGENT.md comme source d'autorité unique pour son rôle, ses limites, son ton et son format de réponse.",
    ],
    technicalRefs: ['agents/<slug>/AGENT.md', 'resolve_agent()', '_perspective_block()'],
    inputs: ['Prompt utilisateur', 'Mode (Auto / Manuel)', 'Profil sélectionné'],
    outputs: ['Agent effectif', 'Instructions système adaptées'],
  },
  {
    title: '4. Fichiers Markdown de contexte',
    subtitle: 'Connaissance éditable injectée selon la demande — budget cible 32 000 car.',
    icon: FileText,
    badges: ['Markdown'],
    details: [
      "Les fichiers de contexte complètent les agents et les skills : mémo consultant, Studio, compte-rendu, migration et notes de version. Chaque fichier est éditable depuis les Paramètres.",
      ...
      "Un budget cible de 32 000 caractères est géré pour le contexte combiné, avec hard cap 36 000. Le contexte projet est tronqué en priorité pour laisser de la place aux données live et au code source.",
    ],
    technicalRefs: ['~/.odoo-consultant/context/', 'consultant-memo.md', 'studio.md', 'creator-conventions.md', 'migration.md', 'meeting-minute.md', 'odoo-v*.md', 'load_context_for_prompt()', '_fit_context_budget()'],
    inputs: ['Fichiers Markdown personnalisés', 'Defaults embarqués', 'Prompt', 'Perspective', 'Version'],
    outputs: ['Sections routées ≤ 32 k cible', 'Notes de version ciblées'],
  },
  {
  ...
  },
  {
    ...
    details: [
      ...
      "Il charge les notes de version Odoo (source + cible si migration), Studio si besoin, le modèle de compte-rendu si la demande le suggère.",
      ...
      "Budget strict : le contexte routé cible 32 000 caractères et reste sous hard cap 36 000 avant d'être envoyé au provider.",
    ],
    technicalRefs: ['load_context_for_prompt()', '_markdown_sections()', '_fit_context_budget()', '_term_matches()', '_BOUNDARY_TOKENS', 'routedContextFiles()', 'CONTEXT_BUDGET=32000', 'MAX_CONTEXT=36000'],
    inputs: ['Dernier prompt', 'Perspective', 'Version source + cible', 'Mode migration'],
    outputs: ['Contexte compact ≤ 32 k cible', 'Budget maîtrisé', 'Notes + Markdown routé'],
  },
  {
...
const stepsEn: DiagramStep[] = [
  ...
  },
  {
    ...
    details: [
      ...
      'Each agent has AGENT.md as the single authority for role, boundaries, tone and response format.',
    ],
    technicalRefs: ['agents/<slug>/AGENT.md', 'resolve_agent()', '_perspective_block()'],
    inputs: ['User prompt', 'Mode (Auto / Manual)', 'Selected profile'],
    outputs: ['Effective agent', 'Adapted system instructions'],
  },
  {
    title: '4. Markdown context files',
    subtitle: 'Editable knowledge injected by the router — 32 000 char target',
    icon: FileText,
    badges: ['Markdown'],
    details: [
      'Context files supplement agents and skills: consultant memo, Studio, meeting minutes, migration and version notes. Each file is editable from Settings.',
      ...
      'A 32 000-character target budget is enforced across the combined context, with a 36 000 hard cap. Project context is truncated first to preserve room for live data and source code.',
    ],
    technicalRefs: ['~/.odoo-consultant/context/en/', 'consultant-memo.md', 'studio.md', 'creator-conventions.md', 'migration.md', 'meeting-minute.md', 'odoo-v*.md', 'load_context_for_prompt()', '_fit_context_budget()'],
    inputs: ['Custom Markdown files', 'Bundled defaults', 'Prompt', 'Perspective', 'Version'],
    outputs: ['Routed sections ≤ 32 k target', 'Targeted version notes'],
  },
  {
  ...
  },
  {
    ...
    details: [
      ...
      'It loads Odoo version notes (source + target in migration mode), Studio when relevant, and the meeting-minute template when the request calls for it.',
      ...
      'Strict budget: routed context targets 32 000 characters and remains under the 36 000 hard cap before being sent to the provider.',
    ],
    technicalRefs: ['load_context_for_prompt()', '_markdown_sections()', '_fit_context_budget()', '_term_matches()', '_BOUNDARY_TOKENS', 'routedContextFiles()', 'CONTEXT_BUDGET=32000', 'MAX_CONTEXT=36000'],
    inputs: ['Latest prompt', 'Perspective', 'Source + target version', 'Migration mode'],
    outputs: ['Compact context ≤ 32 k target', 'Controlled budget', 'Notes + routed Markdown'],
  },
  {
