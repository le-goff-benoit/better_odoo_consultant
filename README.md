...
>
> **Version actuelle :** `0.98.0` — qualité des réponses IA : trois axes adressés en un commit, autour de la même promesse produit (contexte défini et vivant, consultant-first, réponses actionnables). (1) Bloc « Contexte projet » prioritaire unifié dans `runtime-complexity-analyzer` (client + localisation fiscale + profil technique calculé + modules custom + apps installées + stratégie), affiche explicitement « non calculée » quand le diagnostic n'a pas été lancé pour que l'IA ne suppose plus une base vanilla par défaut. (2) Consignes preuves systématiques (fichier:ligne, champ, vue, SHA, traceback) + step further explicite ajoutées aux 4 AGENT.md (FR + EN) ; ton support précis/actionnable/rapide avec alerte custom/Studio, ton BA pédagogique avec concept + mode opératoire + limites. (3) Règle tableaux durcie dans `ai_service.py` : tableau uniquement pour ≥3 éléments × ≥2 dimensions, pas de chaînage ; le mode migration garde le tableau comparatif Source/Cible. Harnais éval : 14 cas qualité + scoring déterministe `criteria_hits` (cite:/step:/context:) + métrique `avg_table_count`. Suite 809 passed. **Précédemment en 0.97.4 :** bugfix édition profil — la clé API n'est plus écrasée quand l'utilisateur sauvegarde sans la modifier. Cause racine dans `update_profile` : `model_dump(exclude_none=True)` n'exclut pas la chaîne vide. Fix une ligne (garde `if v:`) + ceinture+bretelles frontend. +2 tests régressifs. **Précédemment en 0.97.3 :** résolution des 4 faiblesses dispatcher identifiées en 0.97.2 (toutes en xfail avec wording exact du fix). `list` ajouté à `_BOUNDARY_TOKENS` pour stopper la collision verbe "liste" ↔ keyword "list view", nouvelle prune `query-focus` étendue à inspect_view/navigation/security sur list-detail, nouveau pattern `manifest-read-pattern` au scoring, nouvelle rule `pruned:sha-focus` avec préservation de `repo_*` quand override/projet, `financial_terms` enrichi de `rapport tva`/`rapport balance`. Skill dispatcher 311 cas à 100 % / 0 known weakness, agents inchangés à 90.5 %, suite 807 passed. **Précédemment en 0.97.2 :** désambiguïsation des paires de skills sémantiquement proches. 27 cas écrits dans `scripts/quality_eval/disambiguation_cases.jsonl` (3 par paire haute-criticité, nouveau champ `forbidden_skills` qui force « X gagne ET Y/Z perdent »). Le harnais sépare désormais « Real failures » et « Known weaknesses (xfail) ». 4 vraies faiblesses dispatcher identifiées et documentées avec leur classe de fix précise. Accuracy 100 % sur 307 cas, suite 807 passed. **Précédemment en 0.97.1 :** la rigueur du loop appliquée au skill dispatcher. Nouveau harnais unifié `run_skill_dispatcher_eval.py` qui ingère les 28 prompts curated + 256 queries des `skills/*/eval_queries.json` (284 cas) avec split stable train/dev/test, matrice de confusion skill-to-skill, paraphrase accuracy. 24 paraphrases ajoutées sur 12 skills pivots ; 2 trous détectés et bouchés (`odoo_aggregate_records` "CA mensuel", `triage_odoo_error` "exception classer"). Accuracy 100 % sur le corpus de tuning + 100 % paraphrase, suite 807 passed. **Précédemment en 0.97.0 :** refonte de la boucle de feedback du routing agents en 5 itérations mesurables. Split train/dev/test stratifié 70/15/15 + 54 paraphrases sur 27 cas golden, 3 framing detectors orthogonaux (incident / strategy / code-artefact), vote sémantique TF-IDF sur le corpus d'agents, descriptions FR+EN enrichies. Le dispatcher passe d'un faux 100 % d'accuracy (overfit lexical) à un 90.5 % honnête + paraphrase 70.4 % + dev split 93.8 %. Nouveaux outils dans `scripts/quality_eval/` : matrice de confusion par split, `promote_feedback.py` (extrait des candidats du log `routing-feedback.jsonl`), scaffold `llm_judge.py`. Voir le changelog complet dans la page **À propos** de l'app.
